...
export default function Home() {
  const { name, socials, email, about, highlights } = portfolioData.profile;
  const avatarUrl = "/images/dp.jpeg";

  return (
    <>
      <section className="hero">
        <ScrollReveal className="hero-content">
          <img src={avatarUrl} alt={name} className="hero-photo" />
          <h1>{name}</h1>
      ...
      <section className="section" id="about">
        ...
        <div className="about-grid">
          <ScrollReveal animation="fade-left" delay={100} className="about-text glass-card">
            {about.map((paragraph, index) => (
              <p key={index}>{paragraph}</p>
            ))}
          </ScrollReveal>
          <ScrollReveal animation="fade-right" delay={200} className="about-highlights">
